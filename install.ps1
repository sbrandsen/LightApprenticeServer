...
Write-Host -Activity "Downloading zip file" -Status "Step $currentStep of $totalSteps" -PercentComplete (($currentStep / $totalSteps) * 100)
...
# Unzip the file
Write-Host -Activity "Unzipping file" -Status "Step $currentStep of $totalSteps" -PercentComplete (($currentStep / $totalSteps) * 100)
...
Write-Host -Activity "Changing directory" -Status "Step $currentStep of $totalSteps" -PercentComplete (($currentStep / $totalSteps) * 100)
...
Write-Host -Activity "Copying files" -Status "Step $currentStep of $totalSteps" -PercentComplete (($currentStep / $totalSteps) * 100)
...
Write-Host -Activity "Removing files" -Status "Step $currentStep of $totalSteps" -PercentComplete (($currentStep / $totalSteps) * 100)
cd ..
...
Write-Host -Activity "Completed" -Status "All steps have been completed" -Completed
